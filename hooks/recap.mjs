...
async function main() {
  ...
  let pick = sessionId ? candidates.find((c) => c.ids.includes(sessionId)) : null;
  if (!pick && cwd) {
    ...
    else if (same.length > 1) pick = same.sort((a, b) => b.mtime - a.mtime)[0];
  }
  if (!pick) return;
  const acts = pick.entries.filter((e) => e.kind !== 'session');
  if (!acts.length) return;

  const head = 'Computer Use journal for this session, restored after the compaction. The server is still running: '
